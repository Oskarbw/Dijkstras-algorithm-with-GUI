...
void heapifyDown(tHeapQueue* heapQueue, double* pathLength)
{
    ...
    while ((tmp * 2) + 1 < (heapQueue->end)
        && (pathLength[heapQueue->cells[tmp]] > pathLength[heapQueue->cells[(tmp * 2) + 1]]
               || pathLength[heapQueue->cells[tmp]] > pathLength[heapQueue->cells[(tmp * 2) + 2]]))
    {
        if (pathLength[heapQueue->cells[(tmp * 2) + 1]]
            < pathLength[heapQueue->cells[(tmp * 2) + 2]])
        {
...
void heapifyUp(tHeapQueue* heapQueue, double* pathLength)
{
    ...
    double tmpVertex = 0;
    while (
        tmp != 0 && pathLength[heapQueue->cells[(tmp - 1) / 2]] > pathLength[heapQueue->cells[tmp]])
    {
        tmpVertex = heapQueue->cells[tmp];
        ...
        for (int k = 0; k < n; k++)
        {
            ...
            for (int j = 0; j < directions; j++)
            {
                if (graph[currentVertex][j].vertex != -1
                    && (graph[currentVertex][j].weight + pathLength[currentVertex]
                           < pathLength[graph[currentVertex][j].vertex]))
                {
                    pathLength[graph[currentVertex][j].vertex]
                        = graph[currentVertex][j].weight + pathLength[currentVertex];
