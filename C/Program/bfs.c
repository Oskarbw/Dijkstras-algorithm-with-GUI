...
int BFS(t_pair** graph, int n,int startingVertex){
	...
	for (int i=0;i<n;i++){ //initially all vertexes are unvisitied (vertexState=0)
		vertexState[i] = 0;
	}
	...
	while((currentVertex=popFromQueue(queue))!=-2){ //while queue isn't empty
	
		for (int j=0;j<4;j++){
			if (vertexState[graph[currentVertex][j].vertex]==0&&graph[currentVertex][j].vertex!=-1) {
