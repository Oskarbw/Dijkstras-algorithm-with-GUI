...
readArgumentsLoadMode (int argc, char **argv, int *pairN, int **pairs,
		       int *generatePairs)  { FILE * in =
    fopen (argv[2], "r");
  ...
  readArgumentsRandMode (int argc, char **argv, int *rows, int *columns,
		       double *low, double *high, int *pairN, int **pairs,
	...
	
	{
		...
		("Nie udało się wczytać liczby kolumn, lub jest ona niepoprawna. Ustawiono wartość domyślną - 15\n");
	...
	
	{
		...
		("Nie udało się wczytać dolnego zakresu, lub jest on niepoprawny. Ustawiono wartość domyślną - 0\n");
