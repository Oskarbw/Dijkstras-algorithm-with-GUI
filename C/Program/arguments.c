    ...
    int* pairN, int** pairs, int* generatePairs, int* doSave)
{
    for (int i = 2; i < argc; i++)
    {
        if (strcmp(argv[i], "-r") == 0)
        {
            if (atoi(argv[i + 1]) < 1)
            {
                printf("Nie udało się wczytać liczby wierszy, lub jest ona niepoprawna. Ustawiono "
                       "wartość domyślną - 15\n");
                continue;
            }
            *rows = atoi(argv[i + 1]);
        ...
        if (strcmp(argv[i], "-c") == 0)
        {
            if (atoi(argv[i + 1]) < 1)
            {
                printf("Nie udało się wczytać liczby kolumn, lub jest ona niepoprawna. Ustawiono "
                       "wartość domyślną - 15\n");
                continue;
            }
            *cols = atoi(argv[i + 1]);
        ...
        if (strcmp(argv[i], "-low") == 0)
        {
            if (atoi(argv[i + 1]) < 1)
            {
                printf("Nie udało się wczytać dolnego zakresu, lub jest on niepoprawny. Ustawiono "
                       "wartość domyślną - 0\n");
                continue;
            }
        ...
        if (strcmp(argv[i], "-p") == 0)
        {
            if (atoi(argv[i + 1]) < 0)
            {
                printf("Nie udało się wczytać ilości par, lub jest ona niepoprawna. Ustawiono "
                       "wartość domyślną - 5 losowych par\n");
                continue;
            }
            *generatePairs = 0;
    ...
    if (*low > *high)
    {
        printf("Dolny zakres losowania wag jest większy niż górny. Ustawiono standardowe wartości "
               "losowania od 0 do 1\n");
        *low = 0;
        *high = 1;
    }
